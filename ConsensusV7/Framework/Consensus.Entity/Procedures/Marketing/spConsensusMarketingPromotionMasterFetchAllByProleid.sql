if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusMarketingPromotionMasterFetchAllByProleid')
  exec ('create procedure dbo.spConsensusMarketingPromotionMasterFetchAllByProleid as select 1 as temp')
go
alter procedure dbo.spConsensusMarketingPromotionMasterFetchAllByProleid(@PA_PM_MST_PROLE_ID varchar(11) = null output) as 
begin
  select 
    PM_MST_ID,
    PM_MST_NAME,
  ...
  where PM_MST_PROLE_ID = @PA_PM_MST_PROLE_ID
end
go

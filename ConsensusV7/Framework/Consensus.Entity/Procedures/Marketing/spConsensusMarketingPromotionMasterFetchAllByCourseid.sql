if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusMarketingPromotionMasterFetchAllByCourseid')
  exec ('create procedure dbo.spConsensusMarketingPromotionMasterFetchAllByCourseid as select 1 as temp')
go
alter procedure dbo.spConsensusMarketingPromotionMasterFetchAllByCourseid(@PA_PM_MST_COURSE_ID varchar(11) = null output) as 
begin
  select 
    PM_MST_ID,
    PM_MST_NAME,
  ...
  where PM_MST_COURSE_ID = @PA_PM_MST_COURSE_ID
end
go

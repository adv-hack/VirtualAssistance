if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusMarketingPromotionMasterFetchAllByCmpid')
  exec ('create procedure dbo.spConsensusMarketingPromotionMasterFetchAllByCmpid as select 1 as temp')
go
alter procedure dbo.spConsensusMarketingPromotionMasterFetchAllByCmpid(@PA_PM_MST_CMP_ID varchar(11) = null output) as 
begin
  select 
    PM_MST_ID,
    PM_MST_NAME,
  ...
  where PM_MST_CMP_ID = @PA_PM_MST_CMP_ID
end
go

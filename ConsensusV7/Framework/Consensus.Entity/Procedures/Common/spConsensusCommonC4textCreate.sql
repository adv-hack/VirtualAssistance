if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusCommonC4textCreate')
  exec ('create procedure dbo.spConsensusCommonC4textCreate as select 1 as temp')
go
alter procedure dbo.spConsensusCommonC4textCreate(@PA_TEXT_ID varchar(11) = null output, @PA_TEXT_TYPE varchar(4) = null output, @PA_TEXT_OWNER_ID varchar(11) = null output, @PA_TEXT_BODY varchar(max) = null output, @PA_TEXT_RTF varchar(255) = null output, @PA_TEXT_HTML varchar(max) = null output, @PA_TEXT_OWNER_URL varchar(250) = null output, @_AUDIT_USER_ID varchar(3) = null) as 
begin

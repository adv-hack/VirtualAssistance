if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningCourseFetchAllByProvproleid')
  exec ('create procedure dbo.spConsensusLearningCourseFetchAllByProvproleid as select 1 as temp')
go
alter procedure dbo.spConsensusLearningCourseFetchAllByProvproleid(@PA_COURSE_PROV_PROLE_ID varchar(11) = null output) as 
begin
  select 
    COURSE_ID,
    COURSE_LOCK,
    ...
    COURSE_CUT_PRD_TIME
  from dbo.Course
  where COURSE_PROV_PROLE_ID = @PA_COURSE_PROV_PROLE_ID
end
go

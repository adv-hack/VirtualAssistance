if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningCourseFetchAllByPshid')
  exec ('create procedure dbo.spConsensusLearningCourseFetchAllByPshid as select 1 as temp')
go
alter procedure dbo.spConsensusLearningCourseFetchAllByPshid(@PA_COURSE_PSH_ID integer = null output) as 
begin
  select 
    COURSE_ID,
    COURSE_LOCK,
    ...
    COURSE_CUT_PRD_TIME
  from dbo.Course
  where COURSE_PSH_ID = @PA_COURSE_PSH_ID
end
go

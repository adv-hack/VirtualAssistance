if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningCourseFetchAllByFundedorgid')
  exec ('create procedure dbo.spConsensusLearningCourseFetchAllByFundedorgid as select 1 as temp')
go
alter procedure dbo.spConsensusLearningCourseFetchAllByFundedorgid(@PA_COURSE_FUNDED_ORG_ID varchar(11) = null output) as 
begin
  select 
    COURSE_ID,
    COURSE_LOCK,
    ...
    COURSE_CUT_PRD_TIME
  from dbo.Course
  where COURSE_FUNDED_ORG_ID = @PA_COURSE_FUNDED_ORG_ID
end
go

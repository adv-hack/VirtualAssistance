if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningProductFetchAllByMsoff')
  exec ('create procedure dbo.spConsensusLearningProductFetchAllByMsoff as select 1 as temp')
go
alter procedure dbo.spConsensusLearningProductFetchAllByMsoff(@PA_PROD_MS_OFF varchar(11) = null output) as 
begin
  select 
    PROD_ID,
    PROD_LOCK,
    ...
    PROD_GIFTAID
  from dbo.Product
  where PROD_MS_OFF = @PA_PROD_MS_OFF
end
go

if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningProductFetchAllByPshid')
  exec ('create procedure dbo.spConsensusLearningProductFetchAllByPshid as select 1 as temp')
go
alter procedure dbo.spConsensusLearningProductFetchAllByPshid(@PA_PROD_PSH_ID integer = null output) as 
begin
  select 
    PROD_ID,
    PROD_LOCK,
    ...
    PROD_GIFTAID
  from dbo.Product
  where PROD_PSH_ID = @PA_PROD_PSH_ID
end
go

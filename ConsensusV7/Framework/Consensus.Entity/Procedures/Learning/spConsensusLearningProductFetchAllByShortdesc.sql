if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningProductFetchAllByShortdesc')
  exec ('create procedure dbo.spConsensusLearningProductFetchAllByShortdesc as select 1 as temp')
go
alter procedure dbo.spConsensusLearningProductFetchAllByShortdesc(@PA_PROD_SHORT_DESC varchar(1000) = null output) as 
begin
  select 
    PROD_ID,
    PROD_LOCK,
    ...
    PROD_GIFTAID
  from dbo.Product
  where PROD_SHORT_DESC = @PA_PROD_SHORT_DESC
end
go

if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningProductFetchAllByMsrenew')
  exec ('create procedure dbo.spConsensusLearningProductFetchAllByMsrenew as select 1 as temp')
go
alter procedure dbo.spConsensusLearningProductFetchAllByMsrenew(@PA_PROD_MS_RENEW varchar(11) = null output) as 
begin
  select 
    PROD_ID,
    PROD_LOCK,
    ...
    PROD_GIFTAID
  from dbo.Product
  where PROD_MS_RENEW = @PA_PROD_MS_RENEW
end
go

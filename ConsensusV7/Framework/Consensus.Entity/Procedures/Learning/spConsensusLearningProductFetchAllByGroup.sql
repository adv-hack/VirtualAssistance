if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusLearningProductFetchAllByGroup')
  exec ('create procedure dbo.spConsensusLearningProductFetchAllByGroup as select 1 as temp')
go
alter procedure dbo.spConsensusLearningProductFetchAllByGroup(@PA_PROD_GROUP varchar(100) = null output) as 
begin
  select 
    PROD_ID,
    PROD_LOCK,
    ...
    PROD_GIFTAID
  from dbo.Product
  where PROD_GROUP = @PA_PROD_GROUP
end
go

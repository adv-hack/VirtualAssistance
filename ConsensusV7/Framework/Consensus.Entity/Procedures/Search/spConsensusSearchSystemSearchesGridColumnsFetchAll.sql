if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusSearchSystemSearchesGridColumnsFetchAll')
  exec ('create procedure dbo.spConsensusSearchSystemSearchesGridColumnsFetchAll as select 1 as temp')
go
alter procedure dbo.spConsensusSearchSystemSearchesGridColumnsFetchAll as 
begin
  select 
    SYSSC_ID,
    SYSSC_ADD_BY,

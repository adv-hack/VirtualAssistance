if not exists (select object_id from sys.objects where type = 'P' and name = 'spConsensusSearchSystemSearchesGridColumnsFetchById')
  exec ('create procedure dbo.spConsensusSearchSystemSearchesGridColumnsFetchById as select 1 as temp')
go
alter procedure dbo.spConsensusSearchSystemSearchesGridColumnsFetchById(@PA_SYSSC_ID integer = null output) as 
begin
  select 
    SYSSC_ID,
    SYSSC_ADD_BY,
